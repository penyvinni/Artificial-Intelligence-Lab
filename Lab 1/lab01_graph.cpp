...
string get_vertex_label(struct di_graph graph, int vertex_index) { //Δίνοντας την θέση μας επιστρέφει την κορυφή στην αντίστοιχη θέση
  return graph.vertices[vertex_index];
}

int get_vertex_index(struct di_graph graph, string vertex) { //Δίνοντας την κορυφή μας επιστρέφει την θέση της κορυφής αυτής
  int vertex_index{-1};
  for (int i = 0; i < graph.V; i++)
    if (graph.vertices[i].compare(vertex) == 0) {// Συγκρίνει κάθε κορυφή του πίνακα V με της κορυφή στόχο
      vertex_index = i;
      break;
    }
  return vertex_index;
...
int get_weight(di_graph graph, string source_vertex, // Επιστρέφει το βάρος της διαδρομής δίνοντας την κορυφή αφετερίας και την κορυφή τερματισμού
